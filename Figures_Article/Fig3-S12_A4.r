...
strip <- strip_themed(background_x = elem_list_rect(fill = strip_cola),text_x=element_text(size=6))
...
f3a <- ggplot(toplot2,aes(x=TeloName,y=nanoT,col=mut))+
	geom_boxplot(coef=NULL,outlier.shape = NA) +
...
strip <- strip_themed(background_x = elem_list_rect(fill = strip_colb),text_x=element_text(size=10))

f3b <- ggplot(toplot2,aes(x=mut,y=nanoT,fill=mut,group=mut))+
	stat_slab(col=NA,alpha=0.9,scale=0.8,normalize="groups")+
	stat_pointinterval(.width=c(.5,.95),col="black",show.legend=F, point_size = 1.2)+
...
pl <- pl1+pl2 +plot_layout(guides="collect") & theme(legend.position="top") & plot_annotation(title="Figure S12")


#quartz(file=paste0(path2fig,"FigS12.pdf"),height=14,width=8,type="pdf")
#pl
#dev.off()
...
quartz(file=paste0(path2fig,"FigS12_A4.pdf"),height=10.24,width=5.85,type="pdf")
plA4
dev.off()
...
strip <- strip_themed(background_y = elem_list_rect(fill = strip_colL1),text_y=element_text(size=6))
...
strip <- strip_themed(background_y = elem_list_rect(fill = strip_colR1),text_y=element_text(size=6))
...
quartz(file=paste0(path2fig,"Fig3_A4.pdf"),width=10.24,height=5.59,type="pdf")
plA4
dev.off()
