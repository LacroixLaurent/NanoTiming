### Script for Figures 4 and SupFig17 to 21
...
### Raw telomeric data were generated using the telo_tools script from https://github.com/touala/telo_tools
...
#### an intermediate file is provided on Zenodo (https://doi.org/10.5281/zenodo.12668295)
...
f4a <- ggplot(toplot,aes(x=filename,y=seqlen,fill=mut))+
	...
	stat_pointinterval(
		...
		show.legend=F,
		point_size = 0.8,
		interval_size_domain = c(0.5, 0.95),  # Domaine des tailles d'intervalle
		interval_size_range = c(0.25, 0.3))+	geom_text(data=totext,aes(x=filename,y=1100,label=med),col="red",size=2) +
	geom_text(data=totext,aes(x=filename,y=0,label=n),fontface="italic",col="black",size=2) +
	scale_fill_manual("",values=mypal2)+
	ylab("Telomeric repeat length (bp)")

### fig4b and SupFig17-21
...
plWT_all <- ggplot(toplot %>% filter(mut=="WT"))+
	...
	ggtitle("Supplementary Figure 17",subtitle="wt")
...
plRIF1_all <- ggplot(toplot %>% filter(mut=="RIF1"))+
	...
	ylab("RT")+
	xlab("Telomeric repeat length (bp)")+
	ggtitle("Supplementary Figure 18",subtitle="rif1∆")
...
plKU70_all <- ggplot(toplotb %>% filter(mut=="KU70"))+
	...
	ylab("RT")+
	xlab("Telomeric repeat length (bp)")+
	ggtitle("Supplementary Figure 19",subtitle="yku70∆")
...
plCTF19_all <- ggplot(toplot %>% filter(mut=="CTF19"))+
	...
	ylab("RT")+
	xlab("Telomeric repeat length (bp)")+
	ggtitle("Supplementary Figure 20",subtitle="cft19∆")
...
plFKH1_all <- ggplot(toplot %>% filter(mut=="FKH1"))+
	...
	ylab("RT")+
	xlab("Telomeric repeat length (bp)")+
	ggtitle("Supplementary Figure 21",subtitle="fkh1∆")

quartz(file=path2fig %+% "SupFig17-21.pdf",height=10.24,width=7.05,type="pdf")
plWT_all
plRIF1_all
plKU70_all
plCTF19_all
...
strip <- strip_themed(background_y = elem_list_rect(fill = strip_col),text_y=element_text(size=3.5))

plWT_red <- ggplot(toplot_red %>% filter(mut=="WT"))+
	geom_pointdensity(aes(x=seqlen,y=meanBscaled),shape=16,size=0.5)+
	geom_text(data=toplot_red %>% filter(mut=="WT") %>% group_by(teloname) %>% summarise(n=n()),aes(label="n=" %+% n),x=50,y=3,size=2)+
	stat_cor(aes(x=seqlen,y=meanBscaled),show.legend=F,label.x=1000,label.y=c(2.7),method="spearman",cor.coef.name="rho",size=2)+
...
plRIF1_red <- ggplot(toplot_red %>% filter(mut=="RIF1"))+
	geom_pointdensity(aes(x=seqlen,y=meanBscaled),shape=16,size=0.5)+
	geom_text(data=toplot_red %>% filter(mut=="RIF1") %>% group_by(teloname) %>% summarise(n=n()),aes(label="n=" %+% n),x=50,y=3,size=2)+
	stat_cor(aes(x=seqlen,y=meanBscaled),show.legend=F,label.x=1000,label.y=c(2.7),method="spearman",cor.coef.name="rho",size=2)+
	coord_cartesian(ylim=c(0.5,3.5),xlim=c(0,1600))+
	scale_fill_continuous(type = "viridis",direction=-1,option="magma")+
	scale_color_continuous(type = "viridis",direction=-1,option="magma")+
...
plKU70_red <- ggplot(toplot_red %>% filter(mut=="KU70"))+
	geom_pointdensity(aes(x=seqlen,y=meanBscaled),shape=16,size=0.5)+
	geom_text(data=toplot_red %>% filter(mut=="KU70") %>% group_by(teloname) %>% summarise(n=n()),aes(label="n=" %+% n),x=50,y=3,size=2)+
	stat_cor(aes(x=seqlen,y=meanBscaled),show.legend=F,label.x=1000,label.y=c(2.7),method="spearman",cor.coef.name="rho",size=2)+
	coord_cartesian(ylim=c(0.5,3.5),xlim=c(0,1600))+
	scale_fill_continuous(type = "viridis",direction=-1,option="magma")+
	scale_color_continuous(type = "viridis",direction=-1,option="magma")+
...
plCTF19_red <- ggplot(toplot_red %>% filter(mut=="CTF19"))+
	geom_pointdensity(aes(x=seqlen,y=meanBscaled),shape=16,size=0.5)+
	geom_text(data=toplot_red %>% filter(mut=="CTF19") %>% group_by(teloname) %>% summarise(n=n()),aes(label="n=" %+% n),x=50,y=3,size=2)+
	stat_cor(aes(x=seqlen,y=meanBscaled),show.legend=F,label.x=1000,label.y=c(2.7),method="spearman",cor.coef.name="rho",size=2)+
	coord_cartesian(ylim=c(0.5,3.5),xlim=c(0,1600))+
	scale_fill_continuous(type = "viridis",direction=-1,option="magma")+
	scale_color_continuous(type = "viridis",direction=-1,option="magma")+
...
plFKH1_red <- ggplot(toplot_red %>% filter(mut=="FKH1"))+
	geom_pointdensity(aes(x=seqlen,y=meanBscaled),shape=16,size=0.5)+
	geom_text(data=toplot_red %>% filter(mut=="FKH1") %>% group_by(teloname) %>% summarise(n=n()),aes(label="n=" %+% n),x=50,y=3,size=2)+
	stat_cor(aes(x=seqlen,y=meanBscaled),show.legend=F,label.x=1000,label.y=c(2.7),method="spearman",cor.coef.name="rho",size=2)+
	coord_cartesian(ylim=c(0.5,3.5),xlim=c(0,1600))+
	scale_fill_continuous(type = "viridis",direction=-1,option="magma")+
	scale_color_continuous(type = "viridis",direction=-1,option="magma")+
...
quartz(file=paste0(path2fig,"Fig4.pdf"),height=10.24,width=6.14,type="pdf")
plA4
dev.off()
