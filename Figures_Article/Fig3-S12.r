...
strip <- strip_themed(background_x = elem_list_rect(fill = strip_cola),text_x=element_text(size=12))
...
strip <- strip_themed(background_x = elem_list_rect(fill = strip_colb),text_x=element_text(size=16))
...
strip <- strip_themed(background_y = elem_list_rect(fill = strip_colL1),text_y=element_text(size=14))
...
strip <- strip_themed(background_y = elem_list_rect(fill = strip_colR1),text_y=element_text(size=14))
...
pl <- pl1+pl2 +plot_layout(guides="collect") & theme(legend.position="right",axis.title.x=element_text(size=16))

layout <- "
AAAAAAAAB
CCCCCCCCC
"
plo <- f3a+theme(legend.position="none")+f3b+pl+
	plot_layout(design = layout,heights=c(2,5))+
