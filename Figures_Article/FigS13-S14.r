...
fS13a <- ggplot(toplot2bis,aes(x=TeloName,y=nanoT,col=mut))+
...
fS13b <- ggplot(toplot2,aes(x=mut,y=nanoT,fill=mut,group=mut))+
...
AAAAAAAAB
"
plo <- fS13a+theme(legend.position="none")+fS13b+
	plot_layout(design = layout,heights=c(2))+
	plot_annotation(tag_levels = 'a',title="Figure S13")

quartz(file=paste0(path2fig,"FigS13.pdf"),height=4,width=22,type="pdf")
plo
dev.off()
...
pl <- pl1+pl2 +plot_layout(guides="collect") & theme(legend.position="top") & plot_annotation(title="Figure S14")

quartz(file=paste0(path2fig,"FigS14.pdf"),height=14,width=8,type="pdf")
pl
dev.off()
